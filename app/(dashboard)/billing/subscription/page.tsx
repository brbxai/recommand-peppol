...
export default function Page() {
  ...
  const fetchBillingProfile = async () => {
    ...
    try {
      const response = await billingProfileClient[":teamId"][
        "billing-profile"
      ].$get({
        param: { teamId: activeTeam.id },
      });
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      €{currentSubscription.billingConfig.basePrice.toFixed(2)}
                    </p>
                  </div>
                  {currentUsage !== -1 && (
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        {currentUsage} documents were transmitted this month
                      </p>
                    </div>
                  )}
                  <div>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        your current plan features immediately. Billing will be
                        prorated for the current month.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {allPlans.map((plan) => (
            <Card
              key={plan.id}
              className={
                currentSubscription?.planId === plan.id ? "border-primary" : ""
              }
            >
              <CardHeader>
