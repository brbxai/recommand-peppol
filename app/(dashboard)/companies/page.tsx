...
export default function Page() {
  ...
  const handleCompanySubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (dialogMode === 'create') {
        ...
        const json = await handleApiResponse(response, 'Company created successfully');
        ...
        const json = await handleApiResponse(response, 'Company updated successfully');
      ...
      toast.error(`Failed to ${dialogMode === 'create' ? 'create' : 'update'} company`);
    }
  };
  ...
  return <PageTemplate
    ...
    buttons={[
      <Dialog key="create-company-dialog" open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>{dialogMode === 'create' ? 'Create New Company' : 'Edit Company'}</DialogTitle>
          </DialogHeader>
          <CompanyForm
            company={formData}
